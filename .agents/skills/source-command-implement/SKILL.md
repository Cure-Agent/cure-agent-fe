...
1. **스펙 조달**: `node scripts/fetch-spec.mjs <번호>` — 로컬 형제 경로
   (`../cure-agent-be/docs/specs/`)가 있으면 그것을, 없으면 **BE `dev`**의 raw를 가져와
   `.cure-implement/spec-<번호>.md`에 둔다. 못 찾으면 중단한다.
   > **`dev`가 기본인 이유**: BE의 기본 브랜치가 `dev`이고 스펙은 `dev-only`로 착지한다 —
   > **스펙은 항상 dev에 먼저 있고 main에는 다음 `full` 배포가 실어 나른다.** 형제 경로를 쓸
   > 때도 그 레포가 `dev`에 있는지 확인한다(`main` 체크아웃 상태면 오래된 스펙을 읽는다).
...
## Phase 2 — 테스트 동결 (작성: Codex / 리뷰·동결: Codex)
