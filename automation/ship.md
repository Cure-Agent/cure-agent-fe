...
2. 변경 성격에 맞는 타입을 결정한다 (기존 관행: PR 제목 `[FEAT/#11] 히스토리 2-pane 화면 구현`, 브랜치 `feat/11-history-screens`):
   ...
   **`[BUG]`과 `[FIX]`를 가른다 — 세기 위해서다.**
   - `[BUG]` = **운영에서 관측된** 문제. 프로덕션·데모에서 사용자나 로그가 드러낸 것.
   - `[FIX]` = 그 밖의 자체 발견 수정. 배포 전에 잡은 것, 눈으로 본 시각 정리, 계약 정합 등.

   `automation/freeze.md` 서두가 「운영 문제 표본(`[BUG]`)이 아직 0건이라 진단·라우팅 절차를
   세울 근거가 없다 … 표본이 쌓이면 그때 판단한다」로 `/problem` 도입 트리거를 걸어 뒀는데,
   **하나의 `[FIX]`가 두 부류를 같이 담으면 그 트리거는 영원히 울릴 수 없다.** 배지 정렬을
   고친 것과 프로덕션에서 기권한 것을 같은 통에 넣고는 표본을 셀 수 없다.
   `git log --grep='^\[BUG\]'`이 그 표본 목록이 된다.

   **트리거는 울렸다** — 표본 3건(#87·#90·#93)이 쌓여 `automation/problem.md`를 도입했다.
   운영 중 변경은 이제 그 절차가 진입점이고, ship은 그 Phase 6이 위임받는 배포 단계다. 분리를
   계속 지키는 이유도 그대로 남는다: `[BUG]` 표본은 라우팅 게이트가 실제로 옳게 잡고 있는지를
   사후에 되짚는 근거이기도 하다.

   **소급 적용하지 않는다.** 기존 `[FIX]` 7건 중 이 정의에 해당하는 것은 #79(편두통 예시가
   프로덕션 데모에서 기권) 하나뿐이며, 히스토리는 그대로 둔다.

   prefix를 `fix/`와 갈라 둔 이유는 Preflight가 **브랜치 prefix에서 타입을 파싱**하기 때문이다 —
   같은 prefix를 쓰면 자동화가 둘을 구분하지 못한다.
3. 변경 내용을 한 줄로 요약한다.
4. **사용자 확인 대기**:
   ```
   변경사항 요약:
